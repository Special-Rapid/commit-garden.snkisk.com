...
import { PreferencesContext, type Locale, type Theme, readStoredPreference, savePreference, text, usePreferences } from './lib/preferences';
...
function Dashboard({ username }: { username: string }) {
  ...
  return <main className="page-shell dashboard"><UserHeader user={data.user} range={data.range} />{data.calendar.totalContributions === 0 && <EmptyState />}<StatsGrid calendar={data.calendar} stats={data.stats} /><GardenGrid days={data.calendar.days} /><MiniContributionMap days={data.calendar.days} /><ShareButton locale={locale} /></main>;
}
...
function initialTheme(): Theme { const saved = readStoredPreference('theme'); return saved === 'dark' || saved === 'light' ? saved : (matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light'); }
export default function App() { const [path, setPath] = useState(window.location.pathname); const [locale, setLocale] = useState<Locale>(initialLocale); const [theme, setTheme] = useState<Theme>(initialTheme); useEffect(() => { document.documentElement.dataset.theme = theme; document.documentElement.lang = locale; savePreference('theme', theme); savePreference('locale', locale); }, [theme, locale]); useEffect(() => { const listener = () => setPath(window.location.pathname); window.addEventListener('popstate', listener); return () => window.removeEventListener('popstate', listener); }, []); const match = path.match(/^\/u\/([^/]+)$/); const username = match ? decodeUsernamePathSegment(match[1]) : null; return <PreferencesContext.Provider value={{ locale, setLocale, theme, setTheme }}><Header locale={locale} />{match ? username === null ? <InvalidUsernameDashboard /> : <Dashboard username={username} /> : <Landing locale={locale} />}</PreferencesContext.Provider>; }
