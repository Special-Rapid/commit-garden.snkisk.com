...
function Dashboard({ username }: { username: string }) {
  ...
  return <main className="page-shell dashboard"><a className="user-github-link" href={data.user.url} target="_blank" rel="noreferrer">@{data.user.login} · GitHub ↗</a>{data.calendar.totalContributions === 0 && <EmptyState />}<StatsGrid calendar={data.calendar} stats={data.stats} /><GardenGrid days={data.calendar.days} /><ShareButton locale={locale} /></main>;
}
